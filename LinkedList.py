...
class LinkedListPointer(Scene):
    def construct(self):
        ...
        node1 = LinkedListNode("Node1", "1").create()

        # Define the Node 2
        node2 = LinkedListNode("Node2", "2").create().shift(RIGHT * 4)
        ...
        arrow2Label = MathTex("next", color=GREEN_D).next_to(arrow2, UP * 0.5)
        arrow3 = Arrow(start=node1[0].get_right(), end=node2[0].get_left(), color=WHITE)
        arrow3Label = MathTex("next").next_to(arrow3, UP * 0.5)

        self.play(GrowArrow(arrow2), Write(arrow2Label))
        self.play(GrowArrow(arrow3), Write(arrow3Label))
